...
The default VM target is `fc43/vm-default`, and a short target like `fc43` automatically resolves to `fc43/vm-default`. That default target is backed by [fc43/vm-default/vm.conf](/Users/almcwill/Develop/builder/fc43/vm-default/vm.conf) and [fc43/vm-default/cloud-init/user-data](/Users/almcwill/Develop/builder/fc43/vm-default/cloud-init/user-data). Shared `meta-data` and `network-config` live under [config/cloud-init](/Users/almcwill/Develop/builder/config/cloud-init). Add another target by creating a new repo-relative target path such as `fc43/vm-builder/` or `el10/vm-default/`, then pass `--target` or set `VM_TARGET`.
